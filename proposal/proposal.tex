...
i.e., they assign the same supersense to words with different word senses. Supersenses thus provide human-interpretable high-level clustering information, and have mostly been used as features in other tasks, such as preposition sense disambiguation, noun compound interpretation, and metaphor detection \cite{Ye:Baldwin:2007,Tratz:Hovy:2010,Tsvetkov:ea:2013}. 

The task of supersense tagging was first introduced by \cite{Ciaramita:Altun:2006EMNLP}, who used 5-fold cross validation on {\sc SemCor} to evaluate their model (structured 1st order discriminative). Their evaluation also included a held-out development set on each fold that was used to estimate the number of epochs and additional training data containing only verbs. Their overall $F_1$ score on {\sc SemCor} was 77.1.
Reichartz and Paa\ss{} \cite{Reichartz:Paass:2008ECMLPKDD,Paass:Reichartz:2009SIAM} reached an $F_1$ score of 80.2, using CRFs and additional features.
...
diacritics that encode the strength of the MWE being continued (a bar for strong or a tilde for weak).
...
\paragraph{\textsc{Reviews}.} This 55,000-token portion of the English Web Treebank \citep[EWTB;][]{ewtb} 
...
Annotation was done by linguists, who took pains to establish conventions and resolve disagreements.\footnote{Every 
sentence was annotated independently by at least two annotators, and disagreements were resolved through negotiation.} 
...
\paragraph{\textsc{Tweets}.} Two samples of Twitter messages were recently annotated with supersenses \citep{johannsen-14}: 
...
we compute $F_1$ of the supersense part of the joint tag.
...
This is the measure that will be used for competitive ranking of systems.

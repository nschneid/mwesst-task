...
\mc{This task provides a promising framework for integrating lexical semantics in NLP applications. Until recently, it was unclear whether Word Sense Disambiguation could fullfil its goal of improving NLP applications \citep{resnik,carpuat-05}. Successful integrating Word Sense Disambiguation predictions in Machine Translation required reformulating the task: translations in a second language replaced abstract word senses, and the single-word disambiguation targets were replaced by the multiword phrases used by translation systems \citep{chan-07,carpuat-07,levefer-10}. The proposed task learns from these lessons by moving away from fine-grained sense inventories and word-based annotation, while defining a more general representation that can be useful for many NLP applications beyond machine translation.}
...
the}\shortversion{The} lexical semantic analysis for each sentence will be represented as 
...
(\Cref{tbl:reviews} in the annex offers a more detailed breakdown.)
...
We plan to revise the boundaries in the data with greater attention to MWE criteria 
...
All tracks will have access to the data we provide---annotated \textsc{Reviews} and \textsc{Tweets} sentences; 
\longversion{PTB-style part-of-speech}\shortversion{POS} tags\longversion{ at train and test time}; 
and a set of Brown clusters \citep{brown-92} computed from a large corpus---as well as the \longversion{English }WordNet lexicon.}
...
\Cref{fig:linkmeasure} in the annex offers an example sentence and two annotations 
